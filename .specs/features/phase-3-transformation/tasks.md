...
| T4 | Seed config for `county_centroids.csv` (explicit types, `fips` STRING) | `dbt/seeds/_seeds.yml` (or `dbt_project.yml` seed config) | T1 | `dbt seed` loads 474 rows; `fips` stays STRING (no leading-zero loss) |
